...
const ContactField: React.FC<ContactFieldProps> = ({ name, type }) => {
    return (
        <div>
            <p className="text-center text-xl font-light">{name}</p>
            <input className="relative left-1/2 -translate-x-1/2 text-center text-light w-80 py-1 mt-2 mb-5.5 rounded-lg border border-gray-300 dark:border-gray-700" type={type} placeholder={name} />
        </div>
    )
}
...
const Contact = () => {
    return (
        <div className="h-screen relative">
            ...
            <ContactField name="Email" type="text" />
            ...
            <ContactField name="Message" type="text" />
            <button className="relative left-1/2 -translate-x-1/2 rounded-lg bg-blue-500 hover:bg-blue-400 text-white text-center mt-7 p-2 w-80 cursor-pointer transition-all">Submit</button>
